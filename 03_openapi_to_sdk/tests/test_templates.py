...
def test_template_render_snapshot(tmp_path: Path) -> None:
    render_sdk(_sample_ir(), tmp_path)
    ...
    # Snapshot uses key invariants instead of exact spacing to avoid brittle formatting churn.
    assert "from typing import Literal" in generated
    assert "class User(BaseModel):" in generated
    assert 'class_: str | None = Field(default=None, alias="class")' in generated
    assert "Status = Literal['new', 'done']" in generated
    assert "id: int" in generated
    assert expected.splitlines()[0] in generated
